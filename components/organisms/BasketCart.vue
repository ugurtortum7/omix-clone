...
onMounted(async () => {
  ...
  if (!authStore.isAuthenticated) {
    alert('Sepeti görüntülemek için giriş yapmalısınız!')
    router.push('/sign-in')
    return
  }
...
const cartItems = computed(() => basketStore.items)

const cartTotal = computed(() => basketStore.totalPrice)
...
const updateQuantity = async (item: any) => {
  ...
  await basketStore.updateQuantity(item.productId, item.quantity)
}

const removeItem = async (id: string) => {
  const item = cartItems.value.find((item) => item.id === id)
  if (item) {
    await basketStore.removeFromBasket(item.productId)
  }
}
...
const checkout = async () => {
  ...
  try {
    ...
    if (order) {
      ...
      alert(`Siparişiniz başarıyla oluşturuldu! Sipariş No: ${order.id}`)
