  ...
  <div class="purchase-container">
    <div class="purchase-content">
      ...
      <div class="product-details-section">
        ...
        <!-- Add to Cart Button -->
        <button class="add-to-cart-btn" @click="addToBasket">Sepetinize ekleyin</button>
      </div>
    </div>
  </div>
</template>
...
const addToBasket = async () => {
  console.log('🔵 [PurchaseForm] Sepete ekle butonu tıklandı')
  console.log('👤 isAuthenticated:', authStore.isAuthenticated)
  console.log('🎨 Seçili renk:', selectedColor.value.name)
  console.log('💾 Seçili kapasite:', selectedCapacity.value.label)
  ...
  try {
    ...
    // Sepete ekle - seçili renk ve kapasite ile
    await basketStore.addToBasket({
      id: `product-${selectedColor.value.id}-${selectedCapacity.value.id}`,
      productId: `o1-neo-${selectedColor.value.id}`,
      name: `O1 Neo ${selectedColor.value.name} - ${selectedCapacity.value.label}`,
      price: 8999,
      image: selectedColor.value.image
    })
    
    console.log('✅ Ürün eklendi. Toplam items:', basketStore.items.length)
    alert(`O1 Neo ${selectedColor.value.name} sepete eklendi!`)
  } catch (error) {
