    ...
    <div class="sign-in-box">
      ...
      <form @submit.prevent="handleSubmit" class="sign-in-form">
        ...
        <div class="form-group">
          ...
          <div class="captcha-display">UK3HB</div>
          <input
            type="text"
            id="captcha"
            v-model="captcha"
...
const handleSubmit = async () => {
  ...
  if (captcha.value.toUpperCase() !== 'UK3HB') {
    errorMessage.value = 'Güvenlik kodu hatalı!'
    return
  }
  
  const displayName = `${name.value} ${surname.value}`
  const success = await authStore.register(email.value, password.value, displayName)
  
  if (success) {
    alert('Kayıt başarılı! Giriş yapabilirsiniz.')
    router.push('/sign-in')
  } else {
    errorMessage.value = authStore.error || 'Kayıt başarısız!'
  }
}
</script>
